...
Supported providers: Google Drive, Dropbox, OneDrive. For all three, full listings are paged, recursive and pathed relative to the sync folder (`cloud_folder`, default: the whole drive), and change feeds are scoped to that folder. Remote files over the size limit (100 MB by default) are skipped. `/sync` runs a full sync. A file present on both sides whose content matches the provider's hash (Dropbox `content_hash`, OneDrive `sha256Hash` or `quickXorHash`) is left alone; any other goes through the conflict strategy (default: newer wins). A full sync keeps no state between runs, so it can't tell a file deleted remotely from a new local one: the local copy is uploaded again. Google Drive's MD5 isn't compared, so its files on both sides go through the conflict strategy on every run. Delta sync (not yet reachable from the API) reports remote deletions and keeps the local copy without uploading it. When a provider rejects the delta cursor (Dropbox `reset`, Graph `410`), or there is none yet, delta sync takes a fresh cursor and runs a full sync instead; that full sync does not upload files that exist only locally, since they may be remote deletions. Delta sync only brings remote changes down, so with an upload-only direction it does nothing and leaves the cursor as it is. OneDrive follows Graph's `nextLink`/`deltaLink` URLs, and uses a stored delta cursor, only when they are on the configured Graph endpoint (same scheme, host and port); the access token is never sent anywhere else.
