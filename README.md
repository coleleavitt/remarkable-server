...
Upgrading from before #34: a Dropbox or OneDrive `cloud_folder` other than the drive root used to be laid out locally by each file's path from the drive root (`<local>/Notes/a.pdf` for `/Notes/a.pdf`); it is now relative to the folder (`<local>/a.pdf`). The first full sync downloads the folder into the new layout. A local file that looks like a leftover of the old one (the name of a file at the top of the folder, inside local directories the folder doesn't have) is reported in `errors` and not uploaded, since it would land in the folder one level down (`/Notes/Notes/a.pdf`). Remove the old directory once you have kept anything changed in it; see [DEPLOYMENT.md](DEPLOYMENT.md). Whole-drive syncs are unaffected.
