...
struct Inner { conn: Mutex<Connection>, region: String, issuer: String }
...
impl DeviceManager {
    pub fn new<P: AsRef<Path>>(db_path: P, region: &str, issuer: &str) -> Result<Self> {
        let conn = Connection::open(db_path).map_err(|e| ServerError::Database(e.to_string()))?;
        conn.execute_batch("CREATE TABLE IF NOT EXISTS devices (device_id TEXT PRIMARY KEY, device_desc TEXT NOT NULL, registered_at TEXT NOT NULL, last_refresh TEXT NOT NULL, user_id TEXT NOT NULL); CREATE TABLE IF NOT EXISTS pending_codes (code TEXT PRIMARY KEY, user_id TEXT NOT NULL, expires_at TEXT NOT NULL);").map_err(|e| ServerError::Database(e.to_string()))?;
        Ok(Self { inner: Arc::new(Inner { conn: Mutex::new(conn), region: region.into(), issuer: issuer.into() }) })
    }
    ...
    pub fn create_pairing_code(&self, user_id: &str) -> Result<String> {
        ...
        conn.execute("INSERT INTO pending_codes (code, user_id, expires_at) VALUES (?, ?, ?)", params![code, user_id, expires.to_rfc3339()]).map_err(|e| ServerError::Database(e.to_string()))?;
        Ok(code)
    }
    pub fn exchange_code(&self, code: &str, device_id: &str, device_desc: &str) -> Result<(String, String)> {
        ...
        let now = Utc::now();
        conn.execute("INSERT INTO devices (device_id, device_desc, registered_at, last_refresh, user_id) VALUES (?, ?, ?, ?, ?) ON CONFLICT(device_id) DO UPDATE SET device_desc=excluded.device_desc, last_refresh=excluded.last_refresh, user_id=excluded.user_id", params![device_id, device_desc, now.to_rfc3339(), now.to_rfc3339(), user_id]).map_err(|e| ServerError::Database(e.to_string()))?;
        drop(conn);
    ...
    pub fn list_devices(&self) -> Result<Vec<Device>> {
        let conn = self.inner.conn.lock();
        let mut stmt = conn.prepare("SELECT device_id, device_desc, registered_at, last_refresh, user_id FROM devices").map_err(|e| ServerError::Database(e.to_string()))?;
        let devices = stmt.query_map([], |row| Ok(Device { device_id: row.get(0)?, device_desc: row.get(1)?, registered_at: DateTime::parse_from_rfc3339(&row.get::<_, String>(2)?).map(|d| d.with_timezone(&Utc)).unwrap_or_else(|_| Utc::now()), last_refresh: DateTime::parse_from_rfc3339(&row.get::<_, String>(3)?).map(|d| d.with_timezone(&Utc)).unwrap_or_else(|_| Utc::now()), user_id: row.get(4)? })).map_err(|e| ServerError::Database(e.to_string()))?.filter_map(|r| r.ok()).collect();
        Ok(devices)
    }
    pub fn delete_device(&self, device_id: &str) -> Result<bool> { let conn = self.inner.conn.lock(); Ok(conn.execute("DELETE FROM devices WHERE device_id = ?", params![device_id]).map_err(|e| ServerError::Database(e.to_string()))? > 0) }
