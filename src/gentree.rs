...
use axum::{extract::State, http::{header, HeaderMap, StatusCode}, response::{IntoResponse, Response}, Json};
use base64::Engine;
use serde_json::{json, Value};

use crate::{api::AppState, error::{Result, ServerError}, storage::is_valid_hash};

/// JSON field carrying an inline blob body in PutFile.
const BLOB_FIELD: &str = "payload";

fn authz(h: &HeaderMap) -> Result<&str> {
    h.get(header::AUTHORIZATION).and_then(|v| v.to_str().ok()).ok_or(ServerError::Unauthorized)
}

struct IndexEntry { hash: String, id: String, size: u64 }
...
fn parse_index(bytes: &[u8]) -> Vec<IndexEntry> {
    String::from_utf8_lossy(bytes).lines().skip(1).filter_map(|l| {
        let mut it = l.split(':');
        ...
        if hash.is_empty() { None } else { Some(IndexEntry { hash, id, size }) }
    }).collect()
}
...
pub async fn get_files(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<Value>) -> Result<Json<Value>> {
    state.auth_user(&headers)?;
    let h = req_hash(&body, &["hash", "fileHash"]).unwrap_or_default();
    if !is_valid_hash(h) { return Err(ServerError::InvalidHash(h.into())); }
    let idx = state.storage.get(h)?;
    let files: Vec<Value> = parse_index(&idx).into_iter()
...
pub async fn get_file(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<Value>) -> Result<Response> {
    state.auth_user(&headers)?;
    let h = req_hash(&body, &["hash", "fileHash"]).unwrap_or_default();
    if !is_valid_hash(h) { return Err(ServerError::InvalidHash(h.into())); }
...
pub async fn put_file(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<Value>) -> Result<Json<Value>> {
    state.auth_user(&headers)?;
    let file_hash = body.get("fileHash").and_then(|v| v.as_str()).unwrap_or_default();
    if !is_valid_hash(file_hash) { return Err(ServerError::InvalidHash(file_hash.into())); }
    let b64 = body.get(BLOB_FIELD).and_then(|v| v.as_str())
    ...
    let file_path = body.get("filePath").and_then(|v| v.as_str()).unwrap_or("gentree-blob");
    state.storage.put_with_hash(&data, file_hash, file_path)?;
    Ok(Json(json!({"fileHash": file_hash, "sizeBytes": data.len(), "state": "stored"})))
}

/// `POST /gentree/v1/DeleteEntry` req `{entryUuid}` -> tombstone acknowledgement.
pub async fn delete_entry(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<Value>) -> Result<Json<Value>> {
    state.auth_user(&headers)?;
    let uuid = body.get("entryUuid").and_then(|v| v.as_str()).unwrap_or_default().to_string();
...
pub async fn entry_session(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<Value>) -> Result<Json<Value>> {
    let (user_id, device_id, _) = state.devices.caller(authz(&headers)?)?;
    let session_id = body.get("sessionId").and_then(|v| v.as_str()).unwrap_or_default().to_string();
    let pre_gen = body.get("preEntryGeneration").and_then(|v| v.as_u64());
    let post_hash = body.get("postEntryHash").and_then(|v| v.as_str()).unwrap_or_default().to_string();

    let generation = if is_valid_hash(&post_hash) {
        if !state.storage.exists(&post_hash) {
            return Err(ServerError::NotFound(format!("root index {post_hash} not uploaded")));
        }
        let root = state.storage.set_root_if(post_hash, pre_gen)?;
        let _ = state.notification_tx.send(
            crate::notifications::WsMessage::sync_complete(root.generation, &device_id, &user_id));
        root.generation
    } else {
...
mod tests {
    ...
    fn put_body(hash: &str, path: &str, data: &[u8]) -> Value {
        ...
        m.insert("payload".into(), Value::String(base64::engine::general_purpose::STANDARD.encode(data)));
        Value::Object(m)
    }
    ...
    async fn blob_roundtrip_and_session() {
        ...
        let Json(r) = put_file(State(state.clone()), hdrs(&tk), Json(put_body(&blob_hash, "doc/1.rm", payload))).await.unwrap();
        assert_eq!(r["state"], "stored");
        let mut m = serde_json::Map::new();
        m.insert("hash".into(), Value::String(blob_hash.clone()));
        let resp = get_file(State(state.clone()), hdrs(&tk), Json(Value::Object(m))).await.unwrap();
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        assert_eq!(&body[..], payload);

        // Upload a root index that references one document, then commit it via EntrySession
        let root_index = format!("3\n{}:80000000:doc-abc.docSchema:1:{}\n", blob_hash, payload.len());
        let root_hash = "b".repeat(64);
        put_file(State(state.clone()), hdrs(&tk), Json(put_body(&root_hash, "root.docSchema", root_index.as_bytes()))).await.unwrap();
        ...
        let Json(r) = entry_session(State(state.clone()), hdrs(&tk), Json(Value::Object(s.clone()))).await.unwrap();
        ...
        let Json(fr) = get_files(State(state.clone()), hdrs(&tk), Json(Value::Object(gf))).await.unwrap();
        assert_eq!(fr["files"].as_array().unwrap()[0]["fileHash"], blob_hash);

        // A stale preEntryGeneration is rejected (optimistic lock -> 412)
        assert!(entry_session(State(state.clone()), hdrs(&tk), Json(Value::Object(s))).await.is_err());

        // Missing auth is rejected
        assert!(entry_session(State(state.clone()), HeaderMap::new(), Json(json!({}))).await.is_err());
    }
}
