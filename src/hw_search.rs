...
use crate::handwriting::{Points, Word, recognize};
...
impl PageIndex {
    pub fn open(storage: &Storage) -> Result<Self> {
        ...
        conn.execute_batch(
            "CREATE TABLE IF NOT EXISTS pages (rm_hash TEXT PRIMARY KEY, words TEXT NOT NULL)",
        )?;
        Ok(Self {
            db: Mutex::new(conn),
        })
    ...
    fn get(&self, rm_hash: &str) -> Result<Option<Vec<Word>>> {
        let json: Option<String> = self
            ...
            .lock()
            .query_row(
                "SELECT words FROM pages WHERE rm_hash = ?",
                params![rm_hash],
                |r| r.get(0),
            )
            .optional()?;
    ...
    fn put(&self, rm_hash: &str, words: &[Word]) -> Result<()> {
        self.db.lock().execute(
            "INSERT OR REPLACE INTO pages (rm_hash, words) VALUES (?, ?)",
            params![rm_hash, serde_json::to_string(words)?],
        )?;
        Ok(())
    }
}
...
fn page_strokes(data: &[u8]) -> Result<Vec<Points>> {
    ...
    let strokes = remarkable_lines::parse_rm_file(data)
        .map_err(|e| ServerError::Internal(format!("bad .rm page: {e}")))?;
    Ok(strokes
        .into_iter()
        .filter(|s| {
            !s.pen.is_highlighter() && !matches!(s.pen, PenType::Eraser | PenType::EraserArea)
        })
...
fn document_ids(storage: &Storage) -> Result<Vec<String>> {
    ...
    Ok(String::from_utf8_lossy(&storage.get(&root.hash)?)
        .lines()
        .skip(1)
...
async fn page_words(storage: &Storage, index: &PageIndex, rm_hash: &str) -> Result<Vec<Word>> {
    if let Some(words) = index.get(rm_hash)? {
        return Ok(words);
    }
    let strokes = page_strokes(&storage.get(rm_hash)?)?;
    let words = if strokes.is_empty() {
        Vec::new()
    } else {
        recognize(&strokes).await?
    };
...
pub fn spawn_indexer(storage: Storage) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            if root.is_empty() || root == indexed_root {
                continue;
            }
                ...
                for doc in document_ids(&storage)? {
                    for (_, rm_hash) in document_pages(&storage, &doc)? {
                        ...
                        match page_words(&storage, &index, &rm_hash).await {
                            ...
                            Err(e) => {
                                failed += 1;
                                tracing::debug!(%rm_hash, "handwriting index: {e}");
                            }
                        }
                    }
                }
...
fn find_hits(words: &[Word], term: &str) -> Vec<Value> {
    ...
    (0..words.len().saturating_sub(tokens.len() - 1))
        .filter_map(|i| {
            ...
            let matched = tokens
                ...
                .all(|(k, (t, w))| {
                    if tokens.len() == 1 {
                        ...
                        w.ends_with(t.as_str())
                    } else if k == tokens.len() - 1 {
            ...
            (same_line && matched).then(|| {
                ...
                let (x1, y1) = run.iter().fold((f32::MIN, f32::MIN), |(x, y), w| {
