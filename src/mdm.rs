...
pub async fn get_instruction(State(state): State<AppState>, headers: HeaderMap) -> Result<Response> {
    let user = state.auth_user(&headers)?;
    match state.devices.mdm_next_pending(&user)? {
        Some((id, name, key, value)) => {
            let mut data = serde_json::Map::new();
            if let Some(k) = key { data.insert("key".into(), Value::String(k)); }
            if let Some(v) = value { data.insert("value".into(), Value::String(v)); }
            Ok((StatusCode::OK, Json(json!({"id": id, "name": name, "data": Value::Object(data)}))).into_response())
        }
...
pub async fn post_status(State(state): State<AppState>, headers: HeaderMap, Json(body): Json<Value>) -> Result<StatusCode> {
    ...
    if !id.is_empty() { state.devices.mdm_set_status(id, status, detail)?; }
    Ok(StatusCode::OK)
}

#[derive(Deserialize)]
pub struct EnqueueReq { pub name: String, #[serde(default)] pub key: Option<String>, #[serde(default)] pub value: Option<String> }

/// `POST /admin/mdm/enqueue` (admin) -> enqueue a policy command for the device. Returns {id}.
pub async fn admin_enqueue(State(state): State<AppState>, headers: HeaderMap, Json(req): Json<EnqueueReq>) -> Result<Json<Value>> {
    crate::api::require_admin(&headers)?;
    let id = state.devices.mdm_enqueue(LOCAL_USER, &req.name, req.key.as_deref(), req.value.as_deref())?;
    Ok(Json(json!({"id": id})))
}
...
mod tests {
    ...
    async fn queue_poll_and_status() {
        ...
        // nothing queued -> 204
        assert_eq!(get_instruction(State(state.clone()), h.clone()).await.unwrap().status(), StatusCode::NO_CONTENT);

        // enqueue a PIN-length policy, then the device polls and gets it
        let id = state.devices.mdm_enqueue(LOCAL_USER, "minimumPincodeLength", Some("minimumPincodeLength"), Some("6")).unwrap();
        let resp = get_instruction(State(state.clone()), h.clone()).await.unwrap();
        ...
        assert_eq!(get_instruction(State(state.clone()), h.clone()).await.unwrap().status(), StatusCode::OK);

        // device reports completion -> cleared -> 204
        post_status(State(state.clone()), h.clone(), Json(json!({"id": id, "status": "completed"}))).await.unwrap();
        assert_eq!(get_instruction(State(state.clone()), h.clone()).await.unwrap().status(), StatusCode::NO_CONTENT);
    }
}
