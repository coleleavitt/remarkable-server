...
pub async fn post_status(
    ...
    let detail = body
        ...
        .and_then(|v| v.as_str());
    if !id.is_empty() {
        state.devices.mdm_set_status(&user, id, status, detail)?;
    }
    Ok(StatusCode::OK)
}
...
mod tests {
    ...
    async fn status_update_is_scoped_to_owner() {
        ...
        let id = state
            .devices
            .mdm_enqueue(LOCAL_USER, "remoteWipe", None, None)
            .unwrap();
        let mut h = HeaderMap::new();
        h.insert(
            header::AUTHORIZATION,
            HeaderValue::from_str(&format!("Bearer {other}")).unwrap(),
        );
        ...
        assert!(
            !state
                .devices
                .mdm_set_status("someone-else", &id, "failed", None)
                .unwrap()
        );
        ...
        assert!(
            state
                .devices
                .mdm_set_status(LOCAL_USER, &id, "completed", None)
                .unwrap()
        );
        assert!(
            state
