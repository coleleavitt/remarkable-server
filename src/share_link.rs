...
use axum::{extract::{Multipart, Path, State}, http::{header, HeaderMap, StatusCode}, response::{IntoResponse, Response}, Json};
...
pub async fn create(State(state): State<AppState>, headers: HeaderMap, mut form: Multipart) -> Result<Json<Value>> {
    ...
    while let Some(field) = form.next_field().await.map_err(bad)? {
        match field.name().unwrap_or_default() {
            "metadata" => metadata = serde_json::from_str(&field.text().await.map_err(bad)?).unwrap_or(Value::Null),
    ...
    let link = format!("https://{}/share/v1/link/{id}.png", state.devices.get_endpoint());
...
pub async fn get(State(state): State<AppState>, Path(name): Path<String>) -> Result<Response> {
    let id = name.strip_suffix(".png").ok_or_else(|| ServerError::NotFound(name.clone()))?;
